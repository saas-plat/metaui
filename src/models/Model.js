...
export default class Model {
  ...
  @action setProps(props) {
    ...
    _forOwn(props, (defval, key) => {
      ...
      const prikey = '_' + key;
      createProp(this.store, props, prikey, defval);
      Object.defineProperty(props, key, {
        enumerable: true, // 这里必须是可枚举的要不observable不好使
        get: () => {
          // bind就使用bind的vm访问机制
          if (props[getkey]) {
            return this.store.getViewModel(props[getkey]);
          }
          // 因为下面可以修改所以prikey的类型不确定
          return getProp(this.store, props, prikey);
        },
        set: (value) => {
          if (props[setkey]) {
            return this.store.setViewModel(props[setkey], value);
          }
          setProp(this.store, props, prikey, value);
        }
      })
    });
